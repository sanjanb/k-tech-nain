 ...
 */
export async function queueNotification(eventType, dealId, recipientId, channel = NOTIFICATION_CHANNELS.EMAIL) {
  try {
    ...
    if (isDuplicateNotification(existingLogs, eventType, dealId, recipientId)) {
      console.log('Duplicate notification prevented:', { eventType, dealId, recipientId });
      return { isDuplicate: true };
    }
    
    // Create notification log entry
    const logEntry = createNotificationLogEntry(eventType, dealId, recipientId, channel);
    ...
    const docRef = await addDoc(collection(db, 'notification_logs'), firestoreEntry);
 ...
 */
export async function queueDealNotifications(eventType, dealId, buyerId, farmerId) {
  try {
