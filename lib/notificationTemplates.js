...
export function getEmailBodyHTML(eventType, data) {
  const { recipientName, productName, dealId, otherPartyName, platformUrl = 'https://farmtotable.app' } = data;
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <p><strong>${data.recipientRole === 'farmer' ? 'Buyer' : 'Farmer'}:</strong> ${otherPartyName}</p>
        <p><strong>Deal ID:</strong> ${dealId}</p>
      </div>
      
      <p>${data.recipientRole === 'farmer' 
 ...
 */
export function prepareEmailData(eventType, deal, product, recipient, otherParty) {
  return {
    to: recipient.email,
    subject: getEmailSubject(eventType, recipient.role),
    text: getEmailBodyPlainText(eventType, {
    ...
    html: getEmailBodyHTML(eventType, {
      recipientName: recipient.name,
      recipientRole: recipient.role,
      productName: product.cropName,
      dealId: deal.id,
